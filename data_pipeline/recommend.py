...
async def recommend_jobs_for_user(user_id: str, limit: int = 5) -> list:
    ...
    try:
        ...
        if not user_res.data:
            print(f"[추천] {user_id} 유저 없음 (소요: {time.time() - t_prof_start:.2f}초)")
            return []
        user_data = user_res.data[0]
        t_prof_end = time.time()
        print(f"[추천] 1. 유저 프로필 조회 완료 (소요: {t_prof_end - t_prof_start:.2f}초)")
        ...
        t_emb_end = time.time()
        print(f"[추천] 2. 임베딩 벡터 생성 완료 (소요: {t_emb_end - t_emb_start:.2f}초)")
        ...
        t_rpc_end = time.time()
        print(f"[추천] 3. Supabase RPC 호출 완료 (소요: {t_rpc_end - t_rpc_start:.2f}초)")
        ...
        if user_location:
            ...
            print(f"[추천] 지역 필터({user_location}) 후: {len(filtered)}건 (총 소요: {time.time() - t0:.2f}초)")
        ...
        print(f"[추천 로직 통합 예외] {e} (총 소요: {time.time() - t0:.2f}초)")
        return []
