...
수정 및 첨삭 시 아래 [인사담당자들의 조언]을 반드시 100% 반영하여 수정 작업을 진행하십시오. 
사용자의 추가 수정 요청이 조언의 원칙(STAR 기법의 수치화, 구체적 기여 목표, 신중년 위기대응 에피소드)을 훼손하지 않도록 주의하고, 기존에 적용된 조언도 계속 유지해 주세요.

[인사담당자들의 조언]
{tips_context}

수정하지 않는 항목은 기존 내용을 최대한 그대로 유지하고,
반드시 아래 형식으로 출력하세요. 다른 인사말이나 설명은 적지 마십시오.
...
async def revise_resume(
    ...
    response = await llm_smart.ainvoke(
        [
            SystemMessage(content=_REVISION_SYSTEM_PROMPT.format(tips_context=_YOUTUBE_TIPS_CONTEXT)),
...
async def generate_resume_with_tips(
    user_data: dict, job_details: Optional[dict] = None, additional_context: str = ""
) -> str:
...
4. 만약 [지원 희망 채용 공고 정보]가 함께 제공된다면, 해당 회사와 직무 요건(우대사항, 업무 설명 등)을 자소서 항목(특히 지원동기 및 입사 후 포부)에 적극적으로 매칭하여 맞춤형 자기소개서로 가꾸어 주십시오.
...
내용"""
    ...
    if job_details:
        job_context = (
            f"\n[지원 희망 채용 공고 정보]\n"
            f"- 회사명: {job_details.get('company') or '정보 없음'}\n"
            f"- 채용 직무/공고명: {job_details.get('title') or '정보 없음'}\n"
            f"- 주요 업무 및 공고 내용:\n{job_details.get('description') or '정보 없음'}\n"
            f"- 근무지: {job_details.get('location') or '정보 없음'}\n"
        )

    user_prompt = f"""아래 지원자 정보와 조언, 그리고 지원할 채용 공고 정보를 바탕으로 완성도 높은 맞춤형 자소서를 작성해 주세요.
